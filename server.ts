      ...
      const response = await ai.models.generateContent({
        ...
        config: {
          ...
          responseSchema: {
            type: Type.OBJECT,
            properties: {
              name: { type: Type.STRING, description: "A short, creative, futuristic acid-techno name for this patch" },
              cutoff: { type: Type.INTEGER, description: "VCF Filter cutoff frequency in Hz, between 150 and 2600" },
              resonance: { type: Type.NUMBER, description: "Filter resonance Q value, between 1.0 and 13.0" },
              distortion: { type: Type.NUMBER, description: "Overdrive saturation factor, between 0.0 and 1.0" },
              sidechainEnabled: { type: Type.BOOLEAN, description: "Whether this sound pumps with the kick drum using 4/4 sidechain" },
              waveform: { type: Type.STRING, description: "The primary synthesizer oscillator waveform. Must be either 'sawtooth' or 'square'" },
              decay: { type: Type.NUMBER, description: "Envelope and filter decay time in seconds, between 0.05 and 1.2" },
              envMod: { type: Type.NUMBER, description: "Filter envelope modulation sweep depth in octaves, between 0.5 and 6.0" },
              portamento: { type: Type.NUMBER, description: "Glide/slide transition time between notes in seconds, between 0.0 and 0.4" },
              delayFeedback: { type: Type.NUMBER, description: "Feedback amount of the delay echo effect, between 0.0 and 0.85" },
              delayMix: { type: Type.NUMBER, description: "Dry/wet mix level of the delay echo effect, between 0.0 and 0.75" },
              explanation: { type: Type.STRING, description: "A 1-sentence breakdown of how this synthesizes the prompt" }
            },
            required: [
        ...
        config: {
          ...
          responseSchema: {
            ...
            properties: {
              ...
              synthSteps: { type: Type.ARRAY, items: { type: Type.BOOLEAN } },
              synthPitches: { type: Type.ARRAY, items: { type: Type.STRING } },
              kickSteps: { type: Type.ARRAY, items: { type: Type.BOOLEAN } },
              hihatSteps: { type: Type.ARRAY, items: { type: Type.BOOLEAN } },
              clapSteps: { type: Type.ARRAY, items: { type: Type.BOOLEAN } },
              synthSettings: {
                type: Type.OBJECT,
                properties: {
